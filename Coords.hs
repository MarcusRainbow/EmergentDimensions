...
-- |Type alias for a matrix, which may be sparse
type Matrix a = VV.Vector (Vector a)
...
count_dimensions :: Matrix Distance -> Either String Int
...
count_coord_dimensions :: Matrix Coord -> Int
...
-- |dimension first
dimension_ranges :: Matrix Distance -> Either String [Double]
...
foldr_axes :: (Coord -> a -> a) -> a -> Matrix Coord -> [a]
...
foldr_axis :: (Coord -> a -> a) -> Int -> a -> Matrix Coord -> a
...
-- |between points.
validated_coords :: Matrix Distance -> Either String (Matrix Coord)
...
coords :: Matrix Distance -> Either String (Matrix Coord)
...
coords_constructor ::  Matrix Distance ->  Matrix Coord -> Vector Coord
...
calc_coord :: Vector Distance -> Matrix Coord -> Vector Coord
...
validate_non_nan :: Matrix Coord -> Either String (Matrix Coord)
...
coord_constructor :: Vector Distance -> Matrix Coord -> Vector Coord -> Double
...
first_coord_of_dim :: Int -> Matrix Coord -> Vector Coord
...
sum_pair v1 v2 = foldr2 (\x y acc -> acc + x * y) 0.0 0.0 v1 v2

-- |Sum of squares of differences of members in two vectors
sum2_diff :: Vector Double -> Vector Double -> Double
sum2_diff v1 v2 = foldr2 (\x y acc -> acc + (x - y)^2) 0.0 0.0 v1 v2

-- |Utility to fold over two vectors. The length of the longer vector is
-- |used, with the shorter vector being padded with zeroes.
foldr2 :: Unbox a => (a -> a -> b -> b) -> b -> a -> Vector a -> Vector a -> b
foldr2 f acc def x y = 
    case compare nx ny of
        EQ -> foldr_both acc nx
        GT -> foldr_both (foldr_x acc) ny
        LT -> foldr_both (foldr_y acc) nx
    where
        nx = V.length x
        ny = V.length y
        foldr_both acc n = foldr (\i a -> f (x!i) (y!i) a) acc [0..(n-1)]
        foldr_x acc = foldr (\i a -> f (x!i) def a) acc [ny..(nx-1)]
        foldr_y acc = foldr (\i a -> f def (y!i) a) acc [nx..(ny-1)]

-- |Utility to fold over three vectors, which can be different lengths.
-- |The length of the longest vector is used, with the shorter vectors being padded with zeroes.
foldr3 :: Unbox a => (a -> a -> a -> b -> b) -> b -> a -> Vector a -> Vector a -> Vector a -> b
foldr3 f acc def x y z =
    let
        n = V.length x `max` V.length y `max` V.length z
        (!) = get_or def -- redefine ! to not walk off end of vector
    in
        -- if we try to unwrap this into sections, it gets very messy
        foldr (\i a -> f (x!i) (y!i) (z!i) a) acc [0..(n-1)]
...
calc_distance x y = safe_sqrt ((sum2_diff x y))
...
approx_equal_vv :: Double -> Matrix Double -> Matrix Double -> Bool
approx_equal_vv tol x y = 
    VV.length x == VV.length y && 
    VV.and ((VV.zipWith (approx_equal_vector tol)) x y)

match_uncoords :: Double -> Matrix Distance -> Matrix Coord -> Bool
...
-- |a range, and a size.
create_random_triangular_matrix :: StatefulGen g m => g -> (Double, Double) -> Int -> m (Matrix Double)
...
-- |Various tests of foldr3
test_foldr3 :: (Int, Int)
test_foldr3 = let
    v1 = fromList [1, 2, 3]
    v2 = fromList [4, 5, 6, 7]
    v3 = fromList [8] 
    result1 = foldr3 (\x y z a -> a + x * (y + z)) 0 10 v1 v2 v3 -- 12 + 30 + 48 + 170 = 260
    result2 = foldr3 (\x y z a -> a + z * (x + y)) 0 0 v1 v2 v3  -- 40
    in
    assert (result1 == 260 && result2 == 40)
    (result1, result2)
...
test_coords_simplex :: Either String (Matrix Coord)
...
test_coords :: Matrix Distance -> Matrix Coord -> Either String (Matrix Coord)
...
test_coords_4d_simplex :: Either String (Matrix Coord)
...
test_roundtrip_4d_simplex :: Either String (Matrix Coord)
...
test_coords_345 :: Either String (Matrix Coord)
test_coords_345 = let
    sample_dists = fromLists [
        [3.0],
        [4.0, 5.0]]
...
test_coords_345_plus :: Either String (Matrix Coord)
...
test_coords_degenerate_345 :: Either String (Matrix Coord)
...
test_coords_degenerate_345_plus :: Either String (Matrix Coord)
...
test_count_dimensions :: Matrix Distance -> Int -> Either String Int
...
test_count_dimensions_bad :: Matrix Distance -> Either String Int
...
test_uncoords :: Either String (Matrix Distance)
test_uncoords = let
...
main = do
    ...
    putStrLn $ "test_test_foldr3: " ++ (show test_foldr3)
